...
| **AMENDED (issue #8): SIGTERM now drains by default** | ~~SIGTERM gives in-flight checks/hooks zero grace~~ | The original stance — first SIGTERM sends the process group an immediate SIGKILL, no drain window — was correctness-safe by Invariant 4 but behaviorally a crash, a poor default for planned upgrades of a long-running queue that may be one CAS away from landing an expensively-verified merge. **Now the first SIGTERM begins a graceful drain** (config `shutdown "drain"`, the default): the admission boundary gates `refillLane`/`refillSpeculate` (no new candidate, no speculation extension) and suppresses infra auto-retry, so the ALREADY-ADMITTED set — each candidate's remaining required graph plus one landing CAS — is finite; the reconcile loop exits cleanly once its lanes empty, then the entire queued post-land hook backlog runs (hooks have no post-restart replay, so dropping it would be silent permanent loss — bounded by the runner's 64-deep landing buffer, which admission-stopped drain stays well under in practice), then teardown in the normal order. A stale-base landing during drain becomes its terminal skip and is NOT re-admitted (that finiteness is the whole point). A **second signal** (or a `--deadline`/`drain` API deadline) forces the original immediate-kill path — no worse than the old crash, and Invariant 4 still heals it. `shutdown "kill"` restores the legacy first-signal-kills behavior verbatim. Lifecycle (`running`/`draining`/`drained`), the active run/check counts, and the drain window are in the Snapshot and surfaced by `GET /api/v1/status`, the MCP `status` tool, and `POST /api/v1/drain` + `gauntlet drain [--wait]`; signals alone are a complete drain interface when no HTTP admin surface is configured. systemd's `TimeoutStopSec` is the outer hard bound — configure it long enough for expected checks. |
...
| **KEPT** | CLI is a thin HTTP client, deliberately | `status`/`retry`/`cancel`/`hooks-cancel`/`drain`/`land`/`version` only — no `runs`/`run`/`batch`/`checks`/log subcommands. Richer browsing is the dashboard/API/MCP's job (docs/api.md); the CLI exists for the handful of write/porcelain actions worth a bare command, not as a second read surface that has to stay in sync with the other three. |
...
| **KEPT** | Trial-ref publication statuses the tested merge, not the candidate (issue #7) | The commit Gauntlet tests is a synthetic merge that starts life only in its bare repo; a green status on the *candidate* SHA doesn't say which base was verified (the same candidate retried on a newer tip is a different merge). Opt-in `github { trial-refs { prefix; retention } }`: each run CAS-publishes its chain-tip merge under an immutable ref `<prefix>/<run-id>` before checks start, and the rollup status moves to the merge SHA as a VERIFICATION claim — pending on publish (new `EventTrialMerged`, carrying the MergeSHA that `EventTrialClean` can't since it fires pre-CommitTree), success on a new `EventVerified` emitted when the graph goes green **before** the landing CAS (so a "tested SHA green before ref update" policy can exist — posting success only after landing could never satisfy it), failure/error on source/infra. Landing stays the subsequent CAS; `EventLanded` no longer repaints the verification status. **Custom namespace by default** (`refs/gauntlet/trials`, not `refs/heads/**`): the ledger's own "custom namespaces are hostile on hosted remotes" caution was resolved by the spike this issue required — a `refs/gauntlet/*` push to the real repo was accepted, ls-remote-able, Git-API-resolvable, statusable, and triggered no Actions run; `refs/heads/**` is allowed but documented as workflow-trigger-prone. **Synchronous publish in v1** (one CAS round trip per run start, latency on the span) — async-ordered publication's larger state machine waits for a measurement showing the round trip matters. **One ref + one status at the batch chain tip** — a batch verifies one suite over the combined tree, so member candidates get no separate verification statuses (attribution stays in record/dashboard/status URL); a batch that falls back to serial abandons its chain-tip merge, whose pending status is superseded by the per-member serial re-runs and GC'd once its reaped ref makes the merge unreachable. Idempotency is free (git treats a same-SHA re-publish as up-to-date); a ref at a DIFFERENT SHA is an operational collision (never force-overwritten — a trial ref is immutable). Landing CAS-deletes the ref; a non-landing run retains it for `retention` (default 24h) then a per-tick reaper CAS-deletes it (keyed on the SHA, race-safe); crash orphans are swept at boot like GC pins. Both new events and the ref plumbing are gated so disabled-mode is byte-identical to today. Composes with #6's provider; statuses stay on the Statuses API (Checks API migration is separate). |
| **KEPT** | GitHub App auth as one refreshable provider; git credentials via ephemeral askpass (issue #6) | A long-running daemon can't live on a job-scoped token or a startup-read string: `ghauth` mints App JWTs (stdlib RS256 — three fixed claims don't justify a JWT dependency) and exchanges them for installation tokens lazily, cached to a 2-minute pre-expiry window, singleflighted, invalidate-and-retry-ONCE on a clear 401 (never on 403 — permission isn't expiry, and retrying arbitrary auth failures forever is forbidden). One provider feeds both consumers: ghstatus requests a token per POST, and gitx authenticates fetch/push through an ephemeral `GIT_ASKPASS` helper — chosen over `-c http.extraHeader` (argv, ps-visible) and over URL-embedded credentials (the ledger already flags those as readable by checks via the mounted `GAUNTLET_GIT_DIR`). The helper is secretless (token rides in the git subprocess env) and removed on success, failure, and cancellation alike; its host match is ANCHORED on the delimiters that bound a host in git's prompt (`://host` / `@host`, terminated by `'` or `/`), not a bare substring — a first cut used a substring and a review reproduced the token leaking to `github.com.evil.example` through git's default redirect-following. The authed subprocess also runs `LC_ALL=C` so the English-message classification (401 retry, stale-lease detection) can't be defeated by an operator's translated git locale. App mode hard-requires an HTTPS remote canonicalizing to the github block's host **and** owner/repo — mismatch is a startup error, never a silent ambient fallback; static-PAT mode keeps today's independent ambient git behavior, unchanged. Private-key rotation = restart; token refresh ≠ restart. Scope: auth only — statuses API unchanged (Checks API migration is separate later work; #7 composes on this provider). |
