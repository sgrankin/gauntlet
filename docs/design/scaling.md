...
Deliberately pre-need design thinking: nothing here is scheduled. This
exists so the scale-out conversation doesn't restart from zero when load
actually arrives, and so the one real prerequisite (auto-requeue on infra
errors, below) stays visible.
...
## Axis 1 — shard by repo/target (works today)
...
## Axis 2 — park the builder (Azure deallocation; near-zero code)
...
- **Sleep:** driven by the idle signal — `idleSince` (RFC3339, omitted
  while busy) on `GET /api/v1/status`, the MCP `status` tool,
  `gauntlet status`, and a muted line on the dashboard index page. The
  same Function (or a post-idle hook) deallocates once the daemon has been
  idle long enough. Queue idleness (`queue.Snapshot.IdleSince`) is
  composed with hook idleness (no target's post-land hook running or
  backlogged) at the dashboard/MCP layer, since hooks live outside the
  queue package.

Caveat named now so nobody trips on it: deallocation mid-run is a crash
(fine — DESIGN.md Invariant 4's recovery already handles it; hooks may be
recovery-skipped) — the idle signal exists precisely so the Function never
deallocates a busy builder.

## Axis 3 — remote executors (the real scale-out, when one box saturates)
...
- Shared services ([services.md](services.md)) compose per-worker: each
  worker runs its own pool with its own local instances — no cross-worker
  service sharing, for the same reason there's no centralized cache.
...
overnight (axes 1+2), and axis 3 never built.

## The one real prerequisite: auto-requeue on infra errors
...
errors are rare and intolerable when scaling makes them routine.
Auto-requeue-once on error-outcome parks is the connective tissue: it has to
be in place before any axis that makes builders evictable, and it exists —
`auto-retry-errors` (config knob, default **true** — see DESIGN.md's
decision ledger, "Auto-retry once on infra-error parks"), driven through the
same clear-and-emit machinery a human Slack/API/CLI retry uses
(`internal/queue`'s `maybeAutoRetry`/`clearParkAndRetry`). It is
deliberately narrow (infra errors only, once per `(ref, SHA)`, never red
verdicts) so it doesn't reopen the ledger's "no unbounded retry loops"
stance.
...
- No fleet scheduler; axis 3's ceiling is a handful of workers.
