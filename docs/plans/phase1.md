...
4. **Run-ID scheme.** `<UTC yyyymmddThhmmssZ>-<mergeOID[:12]>` — unique across restarts with no persistence (same merge re-tested after restart gets a new timestamp), and human-correlatable to the tested commit.
