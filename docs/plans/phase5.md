...
- **Batch multiplies summary calls per pick:** forming a batch of N makes N
  Claude calls on the reconcile loop *before checks start*. As originally
  shipped these were sequential (stall ≤ N × `summarize.timeout`); the S6
  phase-6-audit fix (`precomputeMergeBodies`, summary.go) runs them
  concurrently capped at 4 in flight, so the stall is now
  ceil(N/4) × `summarize.timeout` — 2× for the default `max-batch 8`, not
  8×. Still the synchronous-summarize stall concern (DESIGN.md watch items),
  just amortized; the standing fix (async summarize off the reconcile loop)
  remains the growth path. Recommend operators running large `max-batch`
  either disable summaries or accept the (reduced) stall.
...
7. **F2 extended to pre-merge failures against a predicted base (post-go-live holistic review, "speculate: a trial conflict against a PREDICTED base parks the candidate, contradicting F2").** Amendment 6 above fixed the false-negative park for a post-merge RED at lane index `i > 0`, but nobody reconciled it with §2.5's own refill-conflict pseudocode ("this candidate parks"), which predates F2 and still describes the unconditional park `startRun`/`rejectPreMerge` implemented for a **pre-merge** verdict — a trial-merge CONFLICT (`!trial.Clean` in `buildChainLink`) — against that exact same kind of predicted, unpushed base. The inconsistency is real: a window `[A, B]` where B's trial-merge against A's predicted `chainTip` conflicts used to park B immediately, sticky, with no auto-retry rescue (`OutcomeConflict` is outside `maybeAutoRetry`'s `OutcomeError`-only scope). If A subsequently goes red at head and never lands, B stays parked forever against a base that never came to exist, even though B may be perfectly landable against the real (unmoved) target tip — exactly the false-negative park amendment 6 exists to prevent, just one step earlier in the pipeline. Fix: `startRun`'s two pre-merge failure branches — trial conflict, and `buildChainLink` returning an infra error outright (MergeTree/CommitTree failing before any trial tree exists) — now check `predicted`. When true, a new `skipPreMergePredicted` (reconcile.go, `rejectPreMerge`'s twin) Skips the candidate unparked (`RunRecord.Outcome = OutcomeSkipped`, `EventSkipped`, no `maybeAutoRetry` call — mirroring `finishRun`'s `park=false` shape exactly) instead of calling `rejectPreMerge`; the candidate's slot is untouched, so it re-queues and re-forms in a later window, retested against whatever base is real by then. When `predicted` is false (lane index 0, base is the live target tip) both branches are byte-for-byte unchanged: `rejectPreMerge` still parks, exactly as before this amendment. The infra-error branch is included deliberately, not just the conflict branch: a `buildChainLink` error against a predicted base is exactly as prediction-tainted as a conflict there (it says nothing about `cand` against the real tip either), and this mirrors the precedent amendment 6 already set on the *post*-merge side — `advanceLane`'s bubble step folds `verdictErrored` into the same no-park branch as `verdictRejected` for `i > 0`, treating a check-time error and a check-time red identically once the base is unproven. `startBatchRun`'s own `rejectPreMerge` call sites (chaining members within one atomic batch, §2.5's "park-and-stop" policy) are deliberately untouched: that chain always lands as one transaction at lane index 0, a materially different design point from a speculation window's independent, multi-run lane, and out of scope for this amendment. Regression coverage: `TestSpeculateConflictAgainstPredictedBase_SkipsRequeuesThenParksOnRealRetest` (rewrite of the old `..._Parks`, proving both the no-park-on-prediction half and the parks-for-real-once-retested-against-reality half) and `TestSpeculateConflictAgainstPredictedBase_LandsWhenPredecessorNeverLands` (the review's exact `[A,B]` scenario: B conflicts with A's predicted chainTip, A reds at head and never lands, B re-tests against the real unmoved tip and lands), both in `internal/queue/speculate_test.go`.
