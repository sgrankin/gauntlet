...
// daemon creates. Mode is derived once from the executor's kind at cmd
// wiring and never varies within one daemon's lifetime — it is recorded
// per instance, not folded into config.ServiceKey, so the key stays pure
// spec identity and reachability is enforced only where it can actually
// vary: at boot adoption (Pool.Adopt rejects a mode-mismatched record).
type Mode int
...
const (
	...
	// via the driver after create. Used by the local executor, which runs
	// checks as host processes with no shared container network to join.
	ModePublish
)
...
// never invents naming, it only executes it.
type InstanceSpec struct {
	Key   string         // full key; becomes the container label
...
// Adopt (from the matched record's spec snapshot): ProbeReady needs the
// ready-command, which lives in the spec and nowhere else on Instance, and
// this also gives Reap per-instance IdleTTL without a registry lookup back
// into config.
...
type Driver interface {
	...
	// adoption can match on it later — never on the name, which only
	// carries the truncated keyhash12.
	Create(ctx context.Context, is InstanceSpec) (Instance, error)

	// ProbeAlive reports existence + running state only — NOT readiness.
	// Used at adoption and at the mid-run-death re-probe, both of which
	// only care whether the instance still exists at all.
	ProbeAlive(ctx context.Context, in Instance) (bool, error)

	// ProbeReady runs in.Spec.ReadyCommand inside the instance (`exec`),
	// or — when ReadyCommand is empty — the default probe.
	ProbeReady(ctx context.Context, in Instance) error

	// Destroy always removes the instance AND its anonymous volumes
	// (`rm -f -v`); specs cannot declare named volumes, so there is
	...
	// never touches the spec, the key, or the harness.
	...
	// Adoption must match by this, never by the name itself — names carry
	// only a 12-hex truncation, and adoption trusts on-box process naming
	// no further than it has to. ok is false, not an error, when name
	// doesn't exist or carries no gauntlet service-key label — both mean
	// "can't adopt this by key", never "adoption failed".
	...
	// diagnostics only — called just before Destroy on a ready-probe
	// failure, never on routine operation.
