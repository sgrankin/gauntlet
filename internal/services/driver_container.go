...
// containerDriver is the Driver: docker or podman via their CLI, reusing
// the operator's existing images and runtime as-is. It deliberately does
// not reuse internal/executor's runtimeSpec/probeRuntime — the two share
// only the binary name and a reachability probe, while this driver's whole
// argv surface (run -d, network create, inspect, exec, rm -v, logs, port)
// is disjoint from the check executor's run --rm.
// internal/executor/container.go is untouched by this package.
...
func createArgs(is InstanceSpec) []string {
	...
	// Verbatim passthrough, omitted entirely when unset — no gauntlet-chosen
	// default fills in for an author who left these unset.
	if is.Spec.Memory != "" {
...
// on stderr is treated as success — docker/podman both fail `network
// create` on a name collision instead of no-op'ing, so this driver
// swallows exactly that one error text itself.
...
// ProbeAlive implements Driver: existence + running state via inspect,
// NOT the ready command. A failed inspect (typically: gone) reports false
// with no error — "not alive" is the ordinary outcome here, not an infra
...
// instance via exec, or — when ReadyCommand is empty — a default probe
// (Service.ReadyCommand's doc in internal/config).
...
// ReadyCommand: a TCP dial of the resolved endpoint by the daemon. That
// works as stated in ModePublish, where the endpoint is a host-published
// 127.0.0.1 port the daemon can dial directly.
//
// In ModeNetwork, though, the daemon process itself is never attached to
// the shared per-service network — only service and check containers join
// it — so it has no route to the alias at all, and a literal "TCP dial of
// the endpoint by the daemon" is impossible to perform from outside a
// container. Rather than require every image to bundle a specific probe
// tool, this execs into the instance itself and reads its own
// /proc/net/tcp for a LISTEN entry on the declared port — equivalent in
// spirit to ProbeAlive-plus-port-open, chosen over "just reuse ProbeAlive"
// because a running-but-not-yet listening service (e.g. still
// initializing its data directory) must still park as not-ready.
...
// removed with the container; specs cannot declare named volumes so there
...
// TailLogs implements Driver: last ~50 lines, failure-path diagnostics
// only. Best-effort — an error is folded into the returned string rather
// than propagated, since TailLogs is always called right before Destroy
// on an already-failing path and must never itself block cleanup.
